...
data <- nhanes_data |> 
  as.data.frame() |> 
  drop_na() |>
  select(chol_med_use, chol_total, bp_sys_mean)

prop <- glm(chol_med_use ~ chol_total, 
    data = data,
    family = binomial())
data <- prop |> 
